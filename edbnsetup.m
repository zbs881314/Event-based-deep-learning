function [edbn, opts] = edbnsetup(edbn, opts)
    ...
    for u = 1 : numel(edbn.sizes)-1
        ...
        edbn.erbm{u}.decay    = opts.decay;  %新加的
        edbn.erbm{u}.momentum = opts.momentum;
        edbn.erbm{u}.f_infl   = opts.f_infl;    %新加的
        edbn.erbm{u}.f_decay  = opts.f_decay;  %新加的
        edbn.erbm{u}.f_alpha  = opts.f_alpha; %新加的
        edbn.erbm{u}.pcd      = opts.pcd; %新加的
        edbn.erbm{u}.sp       = opts.sp;  %新加的
        edbn.erbm{u}.sp_infl  = opts.sp_infl; %新加的

        % Set up constants for the Siegert formula (新加的)
        ...
        if(opts.wtreset)
            edbn.erbm{u}.W  = opts.initscl*randn(edbn.sizes(u+1), edbn.sizes(u));   %不同   
        ...
        % Temporary storage for data and model intermediates as well as
        % fast weights (新加的)
